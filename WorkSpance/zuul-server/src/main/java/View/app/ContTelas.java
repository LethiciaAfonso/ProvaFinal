...
import Telas.RelatorioVC;
import Telas.RelatorioVD;
import Telas.Tela;

public final class ContTelas {
    ...
    private final Map<Integer, Repositoryone<Model>> repositorios = new HashMap<>();

    private static ContTelas instancia;
    ...
    public static ContTelas obterInstancia() {
        if (instancia == null)
            instancia = new ContTelas();
        return instancia;
    }
    ...
    public Repository<Model> obterRepositorio(int paramInt) {
    ...
    private Tela registrarVenda() {
        return (Tela) new RegistroVenda("", 3, obterRepositorio(11));
    }
